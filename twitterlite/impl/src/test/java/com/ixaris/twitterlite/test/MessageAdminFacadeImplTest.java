...
public class MessageAdminFacadeImplTest {
	...
	public void setUp() throws Exception {
		...
		messagesAdminFacadeImpl = CommonsTestUtils.initSingletonClassWithMocks(this, MessageAdminFacadeImpl.class);
	}
	...
	public void testLookUpMessageSuccess(){
		...
		Mockito.when(query.getResultList()).thenReturn(new ArrayList<Message>());
		  
		  
		Set<Message> s = messagesAdminFacadeImpl.lookupMessages(2, 2);
	...
	public void testLookupMessageZero(){
		Set<Message> s = messagesAdminFacadeImpl.lookupMessages(0, 0);
	}
	@Test(expected = IllegalArgumentException.class)
	public void testLookupMessageZero2(){
		Set<Message> s = messagesAdminFacadeImpl.lookupMessages(5, 0);
	}
	
	@Test(expected = IllegalArgumentException.class)
	public void testLookupMessageNegative(){
		Set<Message> s = messagesAdminFacadeImpl.lookupMessages(-5, 5);
	}
	@Test(expected = IllegalArgumentException.class)
	...
	public void testLookUpByMentioningUserOK(){
		TypedQuery<Message> query= Mockito.mock(TypedQuery.class);
		when(em.createNamedQuery("messageByMention",Message.class)).thenReturn(query);
		Mockito.when(query.getResultList()).thenReturn(new ArrayList<Message>());
		
		
		List<Message> ans = messagesAdminFacadeImpl.lookupMessagesMentioningUser("messageByMention", 0, 20);
	...
	public void testLookUpByHashtagsOK(){
		TypedQuery<Message> query= Mockito.mock(TypedQuery.class);
		when(em.createNamedQuery("messageByHashtag",Message.class)).thenReturn(query);
		Mockito.when(query.getResultList()).thenReturn(new ArrayList<Message>());
		...
		Set<Message> ans = messagesAdminFacadeImpl.lookupMessagesByHashtags(list, 0, 20);
	...
	public void testLookUpByHashTagEs(){
		messagesAdminFacadeImpl.lookupMessagesByHashtags(new ArrayList<String>(), 0, 20);
	}
	...
	public void testHashParsingOk(){
		List<String> s = messagesAdminFacadeImpl.extractContentByDelimiter("#thomas huwa il-#king ! #test2", "#");
